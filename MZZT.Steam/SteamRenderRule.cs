	...
	public abstract class SteamRenderRule {
		...
		public static SteamRenderRule Parse(string text) {
			...
			Type renderRule = Assembly.GetExecutingAssembly().GetTypes()
				...
				.Select(x => x.x)
				.FirstOrDefault() ?? throw new FormatException($"Unknown render function {funcName} in renderer call: {text}");
			ConstructorInfo constructor = renderRule.GetConstructors(BindingFlags.Public | BindingFlags.Instance).FirstOrDefault() ?? throw new InvalidOperationException($"Can't find appropriate constructor for {renderRule.Name}!");
			...
			return constructor.Invoke([.. constructorArgs]) as SteamRenderRule;
		}
